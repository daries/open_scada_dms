...
def index():
    ...
    if focus == None:
        focus = "0"
    return render_template('index.html', async_mode=socketio.async_mode, session=session, user_id=session['user_id'],
                           focus=focus)

# API integrasi
@app.route('/integrasi', methods=['POST'])
def handle_json():
    """Integrasi Scada endpoint untuk Aplikasi External .
      ---
      parameters:
        - name: body
          in: body
          type: json
          required: true
          description: data pelanggan yang ada gangguan,
          schema:
            required:
                - sumber
                - idpel
            ...
            properties:
                sumber:
                  type: string
                  description: Asal aplikasi.
                  default: apkt
                idpel:
                  type: string
                  description: ID Pelanggan.
                  default: 520520123456
                pesan:
                  type: string
                  description: Informasi Gangguanya.
                  default: listrik padam
      responses:
        200:
            description: informasi success dari scada system
            schema:
              properties:
                status:
                  type: string
                  description: status seccess atau error
                  default: success
  """
    data = request.json
    logger.info(data.get('idpel'))
    logger.info(data.get('pesan'))
    logger.info(data.get('sumber'))
    publish_event(data.get('idpel'), data.get('sumber'), data.get('pesan'))
...
def ifs_check_online(interval):
    ...
    while True:
        ...
        for ifs in temp_ifs_status:
            if time.time() > (temp_ifs_status[ifs]['lastseen'] + (interval * 4)) and temp_ifs_status[ifs][
                'online'] == True:  # if more than 2x poll-time, set offline
...
def publish_event(element, msg, value):
    ...
    p = Point("event").tag("element", element).time(int(current_time.timestamp() * 1000000),
                                                    write_precision='us').field("message", msg).field("value",
                                                                                                      str(value))
    influxdb_write_api.write(bucket=event_bucket, record=p)
    socketio.emit('add_event_to_table',
                  {"time": current_time.strftime("%Y-%m-%d %H:%M:%S.%f+00:00"), 'element': element, 'msg': msg,
                   'value': value})
...
if __name__ == '__main__':
    ...
    try:
        mongoclient = pymongo.MongoClient(mongodb_host, 27017,
                                          # 'localhost', 27017, <- added mongodb to localhost for resolution of the replicaset, else there is a coonect error
    ...
    try:
        ...
        if ebucket == None:
            ...
            influxdb_bucket_api.create_bucket(bucket_name=event_bucket, description="a bucket for events",
                                              retention_rules=vbucket.retention_rules)
        else:
